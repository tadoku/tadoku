...
**Never rewrite requests to imitate legacy empty-body behavior.** Do not add endpoint-specific middleware, replace request bodies or otherwise bypass the generated decoder to reproduce a legacy empty-body response. Keep the generated decoder's native behavior and record an intentional difference with `skipParity` in the operation's golden-case table.
...
**Seed Kratos once; reset only after marked tests.** HTTP E2Es share a pinned Kratos process and RAM-backed SQLite database, with fixed identities matching the signed fixture subjects. Keep Kratos out of ordinary per-case resets. Tests that may mutate provider state must mark each native/legacy `implementation` with `resetKratos: true`; journeys call `resetKratosAfter(t, api)` before any mutation on the enclosing test. Cleanup restores the full seed after that implementation or whole journey, including failure/cancellation, using a fresh bounded context. Never reset between journey steps. Shared-provider tests stay sequential; a failed reset makes the fixture unusable. Keep provider-schema SQL and process ownership in test-only helpers, and retain exact golden comparisons.
