...
**Require authentication wiring in production construction.** Business route registration must require authentication middleware; startup constructs it from required JWKS configuration with a bounded initial fetch. Keep probes and proxied routes at their existing access boundaries. Endpoint-only contract assembly may explicitly use passthrough authentication. Test authentication goldens at the middleware boundary with a test-only success handler and downstream identity observation, not a business endpoint. Compare real legacy authentication without its authorization stack; mark intentional differences with `skipParity` in the same test table. Keep a separate, small production-router wiring check. Use synthetic signed requests and public JWKS fixtures. JWT clock overrides belong only in scoped, sequential tests and must always be restored.
