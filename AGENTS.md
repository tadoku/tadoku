...
**Maintain Tadoku API import boundaries in both checks.** Read the [import policy](.depolicy.yaml) and the [architecture guide](services/tadoku-api/README.md#import-policies) before adding an import, package or feature. `.depolicy.yaml` is the authoritative policy for every Go file, including tests and inactive build-tag files. Bazel target `visibility` and the [package groups](services/tadoku-api/BUILD.bazel) enforce the local boundaries during builds. Tests follow their package's layer policy; startup and E2E packages are assembly boundaries.

**Change the policy and visibility together.** When adding or moving a Tadoku API package, assign its depolicy rule and give its `go_library` the narrowest matching visibility. A new feature's generated sqlc package must be visible only to that feature. When an import boundary intentionally changes, update `.depolicy.yaml`, the relevant target visibility or package group, and the architecture guide in the same change; explain the new dependency direction in the PR. Do not widen visibility to `public` merely to make a build pass. Keep depolicy in CI while Bazel does not cover imports from public `services/common` packages or files excluded by the active build configuration.

**Verify import-boundary changes** with `bazel run //:gazelle -- -mode=diff`, `bazel run //tools/ci/depolicy`, and `bazel build //services/tadoku-api/...`. A passing Bazel build alone does not establish full policy coverage.
