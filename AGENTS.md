...
**Test endpoint access at the minimum required level, not with a full auth matrix.** For an unprotected endpoint, send a request without credentials. Test the endpoint's response and business behavior against real PostgreSQL. When shared authentication/authorization middleware is introduced, test invalid credentials, role levels, banned users and provider failures once at the middleware boundary, not again for every endpoint. Provider-protocol tests belong with the provider adapter; do not embed ad hoc Keto emulators in endpoint fixtures or start legacy network servers for response comparisons.

**Require authentication wiring in production construction.** Business route registration must require authentication middleware; startup constructs it from required JWKS configuration with a bounded initial fetch. Keep probes and proxied routes at their existing access boundaries. Endpoint-only contract assembly may explicitly use passthrough authentication, but shared authentication goldens must exercise the production router's real middleware path and verify downstream identity propagation. Use synthetic signed requests and public JWKS fixtures; compare legacy authentication without its authorization stack. Keep intentional differences in separate expectations. JWT clock overrides belong only in scoped, sequential tests and must always be restored.

**Prove migrated endpoint parity with shared goldens.** For every migrated operation, run the same SQL setup, HTTP request and golden response through the native and corresponding legacy production handlers in process, using the real domain/repository/query paths. Reset/reseed before each implementation so writes cannot leak between runs. Reuse production route registration; keep legacy assembly and any deterministic clock binding test-only. Do not duplicate goldens, normalize away response differences, or add legacy dependencies to the native runtime. This covers the endpoint contract, not legacy authentication or infrastructure middleware.

**Keep HTTP cases in language-neutral, reviewable files, with an explicit Go test table.** Each case declares `description []string` and `want` as an HTTP status constant. Use `APITestName(operation, want, description...)` for both the subtest name and its fixture path under `testdata/<operation>/<status>_<description>/`. Every case has a separate `setup.sql`, `request.http` and `golden.http`. Do not maintain a separate fixture-name field or discover cases from directories. Load each case's SQL, check the response status against `want`, and compare the complete HTTP response with its golden. Compare status, headers and body directly, without decoding into the same generated types used by the handler. Seed deterministic IDs and freeze business time; normalize only HTTP line endings. Tests must never rewrite goldens automatically. Review contract changes in the files themselves. Keep endpoint assertions out of process-lifecycle tests.
