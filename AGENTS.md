...
**Document durable conventions for the whole application.** Tadoku API is a full application port. State architecture, compatibility and testing rules for every migrated operation, not as properties of whichever endpoint is being implemented now. Keep concrete examples, current coverage and implementation-specific limitations explicitly labeled and separate from those rules. General guidance must not imply that unimplemented operations or untested behavior are already covered.
