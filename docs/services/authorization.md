...
Role changes are performed by writing/removing relation tuples in Keto:

- Add/remove `admins` for admin privileges
- Add/remove `banned` to ban/unban a user

The immersion API currently writes to Keto for these actions (and records DB audit entries).
