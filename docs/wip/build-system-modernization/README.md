...
- [ ] Branch from `main` after Phase 0 is complete.
- [ ] Set Phase 1 to `In progress` and link the pull request.
- [ ] Add the direct shell dependency to `MODULE.bazel`:

  ```starlark
  bazel_dep(name = "rules_shell", version = "0.8.0")
  ```

- [ ] Load `sh_binary` explicitly at the top of the root `BUILD.bazel`:

  ```starlark
  load("@rules_shell//shell:sh_binary.bzl", "sh_binary")
  ```

- [ ] Keep both existing `sh_binary` targets functionally unchanged.
- [ ] Update `rules_go` from 0.61.1 to 0.62.0.
- [ ] Update Gazelle from 0.51.3 to 0.52.2.
- [ ] Keep `rules_pkg` and `rules_oci` at 1.2.0 and 2.3.0.
- [ ] Regenerate the complete lockfile with Bazel 8:

  ```sh
  bazel mod deps --lockfile_mode=update
  ```

- [ ] Run module cleanup and inspect any proposed `MODULE.bazel` changes:

  ```sh
  bazel mod tidy
  ```

- [ ] Commit every lockfile change generated by the pinned Bazel 8 version.
- [ ] Verify that dependency resolution is frozen after regeneration:
...
- [ ] Re-run the production image coverage queries.
- [ ] Run `bazel run --lockfile_mode=error //:load_images` and confirm every
      image still loads.
- [ ] Review the lockfile diff for provenance and completeness, not for
