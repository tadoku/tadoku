...
const initialRecords: AdminRecord[] = [
  ...
  { id: 'announcement-round5', section: 'announcements', title: 'September Round 5 is open', detail: 'The September round is open. Check the contest schedule for start times and the registration deadline.', status: 'Draft' },
  { id: 'announcement-round6', section: 'announcements', title: 'The November round', detail: 'The next official round begins in November. Check the contest schedule for local dates and times.', status: 'Draft' },
...
export function AdminPage() {
  ...
  return <div className="records-admin-layout">
    ...
    <div className="records-admin-main">
      ...
      {section === 'dashboard' ? <>
        <div className="records-admin-stats"><div className="stat"><span>Community members</span><strong>{users.length}</strong></div><div className="stat"><span>Active official round</span><strong>Round 5</strong><small>{currentContest ? formatDateRange(contestStart(currentContest.start), contestEnd(currentContest.end)) : 'Schedule unavailable'}</small></div><div className="stat"><span>Open reports</span><strong>{openReports.length}</strong></div></div>
        ...
        <div className="records-admin-bottom">
          <section className="page-section"><div className="section-heading records-section-heading"><div><h2 className="paper-type-section">Current contest</h2><p className="muted">{currentContest?.title ?? '2026 Round 5'}</p></div><Link className="text-link" to="/contests/round5">Open contest</Link></div><dl className="records-admin-contest"><div><dt>Period</dt><dd>{currentContest ? formatDateRange(contestStart(currentContest.start), contestEnd(currentContest.end)) : 'Schedule unavailable'}</dd></div><div><dt>Registration deadline</dt><dd>{currentContest ? formatDateTime(contestEnd(currentContest.registrationDeadline)) : 'Schedule unavailable'}</dd></div><div><dt>Submitted sample entries</dt><dd>{logs.filter(log => log.submissions.some(item => item.contestId === 'round5')).length}</dd></div></dl></section>
