...
function RecordEditor({ record, section, onSave }: { record?: AdminRecord; section: AdminSection; onSave: (record: AdminRecord) => void }) {
  ...
  return <Modal title={title} trigger={<Button variant={record ? 'outline' : 'default'} leadingIcon={record ? undefined : <PlusIcon className="paper-icon-default" />}>{record ? review ? 'Review' : 'Edit' : title}</Button>} open={open} onOpenChange={next => { if (next) methods.reset(defaults); setOpen(next) }} footer={null}>
    <FormProvider {...methods}><form className="app-form__fields" onSubmit={methods.handleSubmit(values => { onSave({ ...values, id: record?.id ?? `${section}-${crypto.randomUUID()}`, section }); setOpen(false) })}>
...
export function AdminPage() {
  ...
  return <div className="records-admin-layout">
    ...
    <div className="records-admin-main">
      ...
      <header className="page-header records-admin-heading"><div><h1 className="paper-type-page">{names[section] ?? 'Page not found'}</h1><p className="page-lead">{section === 'dashboard' ? 'The community at a glance, and the work that needs attention.' : section === 'reports' ? 'Review the context before resolving a report.' : section === 'languages' ? 'Supported languages and requests from the community.' : section === 'users' ? 'People making room for reading and listening.' : section === 'contests' ? 'Official rounds and community challenges.' : 'Keep Tadoku’s information clear and useful.'}</p></div>{section === 'dashboard' ? <RecordEditor section="announcements" onSave={saveRecord} /> : ['posts', 'pages', 'announcements', 'languages'].includes(section) ? <RecordEditor section={section as AdminSection} onSave={saveRecord} /> : null}</header>
      {notice ? <Flash variant="success" className="records-admin-notice">{notice}</Flash> : null}
      {section === 'dashboard' ? <>
        <div className="records-admin-stats"><div className="stat"><span>Community members</span><strong>{users.length}</strong><small>In the sample community</small></div><div className="stat"><span>Active official round</span><strong>Round 5</strong><small>1–30 September 2026</small></div><div className="stat"><span>Open reports</span><strong>{openReports.length}</strong><small>Ready for review</small></div></div>
        <section className="page-section records-admin-attention"><div className="section-heading records-section-heading"><div><h2 className="paper-type-section">Needs attention</h2><p className="muted">Review the record, then choose what happens next.</p></div><Link className="text-link" to="/admin/reports">View reports</Link></div><ul className="records-admin-queue">
        ...
        <div className="records-admin-bottom">
          ...
          <section className="page-section"><div className="section-heading records-section-heading"><div><h2 className="paper-type-section">Quick actions</h2><p className="muted">Common workspace tasks</p></div></div><div className="records-admin-quick"><RecordEditor section="posts" onSave={saveRecord} /><RecordEditor section="pages" onSave={saveRecord} /><Link className={buttonClassName({ variant: 'outline' })} to="/admin/languages">Languages</Link><Link className={buttonClassName({ variant: 'outline' })} to="/admin/users">Users</Link></div></section>
        </div>
      ...
      </> : section === 'users' ? <Table caption="Community members" minWidth="32rem" columns={[{ id: 'name', header: 'Member', rowHeader: true, cell: item => <Link className="text-link" to={`/users/${item.id}`}>{item.name}</Link> }, { id: 'joined', header: 'Tracking since', cell: item => formatDate(item.joined) }, { id: 'action', header: 'Activity', cell: item => <Link className="text-link" to={`/users/${item.id}/activity`}>View activity</Link> }]} rows={users} getRowKey={item => item.id} /> : section === 'contests' ? <Table caption="Contests" minWidth="38rem" columns={[{ id: 'title', header: 'Contest', rowHeader: true, cell: item => <Link className="text-link" to={`/contests/${item.id}`}>{item.title}</Link> }, { id: 'type', header: 'Collection', cell: item => item.unlisted ? 'Unlisted' : item.scope === 'official' ? 'Official' : 'Community' }, { id: 'dates', header: 'Begins', cell: item => formatDate(item.start) }, { id: 'action', header: 'Details', cell: item => <Link className="text-link" to={`/contests/${item.id}`}>Open contest</Link> }]} rows={contests} getRowKey={item => item.id} /> : ['posts', 'pages', 'announcements', 'languages', 'reports'].includes(section) ? <Table caption={names[section]} minWidth="32rem" columns={[{ id: 'title', header: section === 'languages' ? 'Language or request' : 'Title', rowHeader: true, cell: item => <div className="records-admin-record"><strong>{item.title}</strong><p>{item.detail}</p></div> }, { id: 'status', header: 'Status', cell: item => <span className="status-tag">{item.status}</span> }, { id: 'action', header: 'Action', cell: item => <RecordEditor record={item} section={item.section} onSave={saveRecord} /> }]} rows={records.filter(record => record.section === section)} getRowKey={item => item.id} emptyMessage={`No ${names[section].toLocaleLowerCase()} yet.`} /> : <section className="empty-state"><h2 className="paper-type-section">This admin page isn’t available</h2><Link className={buttonClassName()} to="/admin">Return to dashboard</Link></section>}
    </div>
  </div>
}
