...
import { Button, Select, Surface, Tabs } from 'paper-ui'
...
export function ComponentWorkbench({
  ...
  const methods = useForm<WorkbenchFields>({
    defaultValues: { fixtureId: fixtures[0]?.id ?? '' },
  })
  ...
  useEffect(() => {
    ...
    if (
      !fixtures.some((candidate) => candidate.id === selectedFixtureId)
    ) {
      methods.reset({ fixtureId: fixtures[0]?.id ?? '' })
    }
  }, [fixtures, methods])
      ...
      >
        <Tabs.Root defaultValue="preview">
          <div className="component-workbench__toolbar">
            ...
            {fixtures.length > 1 ? (
              <div className="component-workbench__fixture-select">
                <Select
                  name="fixtureId"
                  label="Fixture"
                  options={fixtures.map((candidate) => ({
                    value: candidate.id,
