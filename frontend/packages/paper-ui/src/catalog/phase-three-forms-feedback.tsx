  ...
  return (
    <FormProvider {...methods}>
      <form className="paper-fixture-form" noValidate onSubmit={methods.handleSubmit(() => undefined)}>
        ...
        {kind === "radio-select" ? <RadioSelect name="measure" label="Measure progress in" options={[{ value: "pages", label: "Pages" }, { value: "minutes", label: "Minutes" }]} /> : null}
...
function sections(spec: DocSpec): ComponentDocumentationSections {
  const required: RequiredComponentSections = {
    ...
    variants: { heading: "Variants", content: [`Use only the variants exposed by ${spec.react.join(" and ")}; visual differences must represent a documented semantic job.`] },
    statesAndAdaptation: { heading: "States and adaptation", content: ["Review empty, populated, disabled, invalid, long-content, light/dark, comfortable/compact, and narrow states where applicable."] },
    behavior: { heading: "Behavior", content: [spec.behavior] },
    contentGuidance: { heading: "Content guidance", content: [`Use concrete Tadoku nouns and short recovery-oriented messages in ${spec.name}.`] },
...
export const phaseThreeFormsFeedbackFixtures = [
  ...
  fixture("radio-select.measure", "Progress measure", "A compact native radio choice.", `import { RadioSelect } from "paper-ui";\n\n<RadioSelect name="measure" label="Measure progress in" options={measures} />`, () => <FormFixture kind="radio-select" />),
...
const specs: readonly DocSpec[] = [
  ...
  { id: "component.radio-select", route: "/components/forms/radio-select", name: "RadioSelect", category: "forms", summary: "Shows concise mutually exclusive options as native inline radios.", fixtureId: "radio-select.measure", sourcePath: "src/components/forms/controls.tsx", react: ["RadioSelect"], types: ["RadioSelectProps", "Option"], when: "Use for two to four short choices that benefit from remaining visible.", avoid: "Use Select for longer sets and RadioGroup when each choice needs description copy.", choose: "RadioSelect is compact; RadioGroup uses cards for explanatory choices.", behavior: "Arrow keys move within the native same-name radio set and Space selects.", accessibility: "A fieldset and legend name the group while each label names its native radio.", migration: "Preserve submitted option values and replace legacy layout classes." },
  ...
  { id: "component.flash", route: "/components/feedback/flash", name: "Flash", category: "feedback", summary: "Places persistent contextual feedback beside the content it describes.", fixtureId: "flash.statuses", sourcePath: "src/components/feedback/feedback.tsx", react: ["Flash"], types: ["FlashProps", "FlashVariant"], when: "Use for page or section feedback that should remain until the context changes.", avoid: "Use Toast for brief asynchronous confirmation and inline field errors for field recovery.", choose: "Flash is persistent and contextual; Toast is transient and global.", behavior: "Danger feedback uses an urgent alert; other statuses use polite status announcements.", accessibility: "Pair the colored edge with title and message text, and provide actions as real links or buttons.", migration: "Map error to danger, remove Next Link ownership, and pass any action as router-owned React content." },
