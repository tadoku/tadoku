...
const autocompleteProps: readonly Prop[] = [...fieldProps,
...
const details: Record<string, { example: string; anatomy: string; variants: string; states: string; content: string; implementation: string; props: readonly Prop[] }> = {
  ...
  "component.autocomplete": {
    ...
    states: "Search an unmatched word to see No matching options. required validates the selected object, and blur reaches React Hook Form validation modes. disabled prevents both input and trigger interaction.",
  ...
  "component.multi-autocomplete": {
    example: "Add languages from the known collection, remove a named chip, and try the two-selection limit. The empty example demonstrates that required means at least one selected object.",
    ...
    states: "At the selection limit the input and trigger stop accepting additions while removal stays available. Removing one chip re-enables search. The component does not create unknown objects or fetch remote data.",
    content: "State a selection limit in the hint before the user reaches it. Use labels that still make sense in Remove Japanese removal actions.", implementation: formSetup,
