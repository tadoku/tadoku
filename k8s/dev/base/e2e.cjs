...
try {
  ...
  if (envValue(apiContainer, 'API_LEADERBOARD_SHARED_READINESS') !== 'true' || envValue(branchApi, 'API_LEADERBOARD_SHARED_READINESS') !== 'true') throw new Error('Generic worker readiness must gate cache reads during transfer')
  if (envValue(workerContainer, 'WORKER_POSTGRES_DATABASE') !== 'tadoku' || envValue(workerContainer, 'WORKER_LEADERBOARD_CACHE_PREFIX') !== undefined) throw new Error('Base worker must use the base database and unprefixed cache')
