...
func TestIsRole_Match(t *testing.T) {
	ctx := context.WithValue(context.Background(), domain.CtxSessionKey, &domain.SessionToken{Role: domain.RoleAdmin})
	assert.True(t, domain.IsRole(ctx, domain.RoleAdmin))
}

func TestIsRole_NoMatch(t *testing.T) {
	ctx := context.WithValue(context.Background(), domain.CtxSessionKey, &domain.SessionToken{Role: domain.RoleUser})
