...
func (m *mockOutboxRepository) ProcessOutboxBatch(ctx context.Context, batchSize int32, fn func(events []postgres.FetchAndLockOutboxEventsRow) []int64) error {
	if m.batchErr != nil {
		return m.batchErr
	}
	ids := fn(m.events)
...
func (m *mockOutboxRepository) CleanupProcessedOutboxEvents(ctx context.Context, before time.Time) error {
	m.cleanedUp = true
	return m.cleanupErr
}

type mockOutboxLeaderboardUpdater struct {
	contestCalls  []mockOutboxContestCall
	officialCalls []mockOutboxOfficialCall
}

type mockOutboxContestCall struct {
	ContestID uuid.UUID
	UserID    uuid.UUID
}

type mockOutboxOfficialCall struct {
	Year   int
	UserID uuid.UUID
}

func (m *mockOutboxLeaderboardUpdater) UpdateUserContestScore(ctx context.Context, contestID uuid.UUID, userID uuid.UUID) {
	m.contestCalls = append(m.contestCalls, mockOutboxContestCall{ContestID: contestID, UserID: userID})
}

func (m *mockOutboxLeaderboardUpdater) UpdateUserOfficialScores(ctx context.Context, year int, userID uuid.UUID) {
	m.officialCalls = append(m.officialCalls, mockOutboxOfficialCall{Year: year, UserID: userID})
}
...
func TestOutboxWorker_ProcessEvent(t *testing.T) {
	...
	t.Run("processes refresh_contest_score events", func(t *testing.T) {
		updater := &mockOutboxLeaderboardUpdater{}
		...
		worker := domain.NewOutboxWorker(repo, updater, time.Second)
