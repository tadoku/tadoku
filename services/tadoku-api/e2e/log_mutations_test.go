...
func TestImmersionLogCreate(t *testing.T) {
	tests := []struct {
		...
		{description: []string{"authoritative", "missing", "platform"}, want: http.StatusInternalServerError, scoringEngineEnabled: true},
		{description: []string{"invalid", "tags", "before", "registration"}, want: http.StatusInternalServerError},
...
func TestImmersionLogUpdate(t *testing.T) {
	tests := []struct {
		...
		{description: []string{"frozen", "after", "invalid", "tags"}, want: http.StatusInternalServerError},
