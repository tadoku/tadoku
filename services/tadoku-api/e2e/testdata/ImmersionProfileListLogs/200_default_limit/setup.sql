...
delete from log_tags; delete from contest_logs; delete from logs;
insert into logs (id,user_id,language_code,log_activity_id,duration_seconds,eligible_official_leaderboard,created_at,updated_at)
select ('aaaaaaaa-aaaa-4aaa-8aaa-' || lpad(n::text,12,'0'))::uuid, '11111111-1111-4111-8111-111111111111', 'jpn', 1, n, false, '2026-01-01'::timestamp+n*interval '1 minute', '2026-01-01' from generate_series(1,105) n;
insert into contest_logs (contest_id,log_id,duration_seconds,computed_score) select 'f1111111-1111-4111-8111-111111111111', id, duration_seconds, duration_seconds::real from logs;
