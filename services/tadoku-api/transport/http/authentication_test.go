...
func TestAuthenticationRequiresConfiguration(t *testing.T) {
	...
	_, err := NewHandler(app.New(nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil), func(context.Context) error { return nil }, time.Second, prometheus.NewRegistry(), slog.Default(), nil, passthrough, passthrough)
	if err == nil {
		t.Error("router accepted missing authentication middleware")
	}
	_, err = NewHandler(app.New(nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil), func(context.Context) error { return nil }, time.Second, prometheus.NewRegistry(), slog.Default(), passthrough, nil, passthrough)
	if err == nil {
		t.Error("router accepted missing banned-user middleware")
	}
	_, err = NewHandler(app.New(nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil), func(context.Context) error { return nil }, time.Second, prometheus.NewRegistry(), slog.Default(), passthrough, passthrough, nil)
	if err == nil {
...
func TestNewApplicationRoutesInheritSharedMiddleware(t *testing.T) {
	...
	router, err := NewHandler(
		app.New(nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil),
