...
func TestHandlerProxiesEachLegacyPrefix(t *testing.T) {
	...
	for _, name := range []string{"immersion", "profile"} {
		name := name
	...
	if !proxySampleFound {
		t.Error("proxy request histogram sample not found")
	}
	for _, test := range tests {
...
func TestHandlerGeneratesAndForwardsCorrelationID(t *testing.T) {
	...
	handler.ServeHTTP(response, httptest.NewRequest(stdhttp.MethodGet, "/profile/ping", nil))
...
func TestRegisterProxyRoutesRejectsInvalidConfiguration(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "upstream path", upstreams: Upstreams{Immersion: "http://immersion/base", Profile: valid.Profile}, timeout: time.Second},
		{name: "timeout", upstreams: valid},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			err := RegisterProxyRoutes(newProxyTestRouter(prometheus.NewRegistry()), test.upstreams, stdhttp.DefaultTransport, test.timeout, slog.Default())
			if err == nil {
...
func newProxyTestRouter(registerer prometheus.Registerer) *Router {
	duration, err := newRequestDuration(registerer)
	if err != nil {
		panic(err)
	}
	return &Router{rootMux: stdhttp.NewServeMux(), requestDuration: duration}
}
